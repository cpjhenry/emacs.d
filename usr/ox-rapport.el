...
;; reports.  It recognizes English metadata names, despite its French
;; name.
;;
;; Example:
;;
...
;;
;; Attendance is ordinary Org content rather than document metadata:
;;
;;   * Attendance
;;
;;   ** Present
;;
;;   Paul, Jimmy, Dax, Tyler, Ken, Joe, Greg, Jordan, and Ted.
;;
;;   ** Regrets
;;
;;   None recorded.
...
\\definecolor{rapportgray}{HTML}{7F756B}
\\definecolor{rapportlightgray}{HTML}{E5E0DA}
...
;; Replace an older registration whenever this file is reloaded.
...
(defun ox-rapport--value (key info)
  "Return the exported value of KEY from INFO, or nil when empty."
...
(defun ox-rapport--raw-value (key info)
  "Return the unexported value of KEY from INFO, or nil when empty.

This is intended for metadata whose value is not document prose, such
...
(defun ox-rapport--duration (start end)
  ...
  (let ((from (ox-rapport--minutes start))
    ...
    (when (and from to)
      ;; Permit a meeting to continue past midnight.
      (when (< to from)
        (setq to
              (+ to (* 24 60))))
      (let* ((minutes (- to from))
...
(defun ox-rapport--meeting-line (date start end duration)
  "Construct the displayed meeting line from DATE, START, END, and DURATION."
  (let ((when
         (cond
          ((and date start end)
           (format "%s · %s--%s"
...
(defun ox-rapport-template (contents info)
  "Return a complete LaTeX document for CONTENTS and INFO."
  (let* ((title
